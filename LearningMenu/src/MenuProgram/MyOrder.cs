...
namespace MenuProgramEB
{
    public partial class MyOrder : Form
    {
        ...
        private void finishOrder_Click(object sender, EventArgs e)
        {
            ...
            items.Load(@ClassGlobals.RunningPath + "Resources\\Orders\\Order" + (ClassGlobals.ORDER_ID + 1).ToString() + ".xml");
            foreach (XmlNode node in items.DocumentElement)
            {
                String index = node.Attributes[0].InnerText;
                if (index == "State")
            ...
            items.Save(@ClassGlobals.RunningPath + "Resources\\Orders\\Order" + (ClassGlobals.ORDER_ID + 1).ToString() + ".xml");
            this.Close();
        }
